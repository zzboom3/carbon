...
//    private RestHighLevelClient restHighLevelClient;
    ...
    public CarbonArticleQueryVo getCarbonH5ArticleById(Serializable id) {
        ...
        Integer browseNum = byId.getBrowseNum() == null ? 0 : byId.getBrowseNum();
        byId.setBrowseNum(browseNum + 1);
        ...
        List<CarbonArticleCommentQueryVo> comments = commentMapper.selectList(cwrapper)
                .stream()
                .map(e -> {
                    LambdaQueryWrapper<CarbonArticleComment> childWrapper = new LambdaQueryWrapper<>();
                    childWrapper.eq(CarbonArticleComment::getParentId, e.getId());
                    Integer count = commentMapper.selectCount(childWrapper);
                    CarbonArticleCommentQueryVo v = new CarbonArticleCommentQueryVo();
                    BeanUtil.copyProperties(e, v);
                    v.setHavaChild(count != null && count > 0);
                    return v;
                })
                .collect(Collectors.toList());
        LambdaQueryWrapper<CarbonArticleComment> countWrapper = new LambdaQueryWrapper<>();
        countWrapper.eq(CarbonArticleComment::getArticleId, id);
        vo.setCommentNumber(commentMapper.selectCount(countWrapper));
        if (byId.getCreatorId() != null && byId.getCreatorId() != 0) {
            SysAccount account = accountMapper.selectById(byId.getCreatorId());
            if (account != null) {
                vo.setAuthorHeadPortrait(account.getAvatar());
            }
        }
        vo.setCommentList(comments);
        vo.setContent(carbonH5ArticleMapper.getContentById(byId.getId()));
        return vo;
    }

    @Override
    public Paging<CarbonArticleQueryVo> searchByKeyword(CarbonH5ArticleController.H5SearchDefaultParam param) {
        CarbonH5ArticleController.H5SearchDefaultParam query = param == null ? new CarbonH5ArticleController.H5SearchDefaultParam() : param;
        CarbonArticleQueryParam q = new CarbonArticleQueryParam();
        q.setCurrent(query.getCurrent());
        q.setSize(query.getSize());
        q.setSearchKeywords(query.getKeyword());
        return getCarbonArticlePageList(q);
    }
    ...
    public Paging<CarbonMethodologyContent> searchMethodologyByKeyword(CarbonH5ArticleController.H5SearchMethodologyParam param) {
        CarbonH5ArticleController.H5SearchMethodologyParam query = param == null ? new CarbonH5ArticleController.H5SearchMethodologyParam() : param;
        Page<CarbonMethodologyContent> page = new Page<>(query.getCurrent(), query.getSize());
        IPage<CarbonMethodologyContent> iPage = carbonH5ArticleMapper.searchMethodologyPage(page, query);
        return new Paging<>(iPage);
    }

    @Override
    public Paging<CarbonArticleQueryVo> searchArticleByKeyword(CarbonH5ArticleKeyWordSearchParam param) {
        CarbonH5ArticleKeyWordSearchParam query = param == null ? new CarbonH5ArticleKeyWordSearchParam() : param;
        CarbonArticleQueryParam q = new CarbonArticleQueryParam();
        q.setCurrent(query.getCurrent());
        q.setSize(query.getSize());
        q.setCategoryId(query.getCategoryId());
        q.setSearchKeywords(query.getKeyword());
        return getCarbonArticlePageList(q);
    }
