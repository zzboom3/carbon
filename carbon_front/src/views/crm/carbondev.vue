      ...
      if (!code) {
       ...
       this.sendCode();
          ...
          getFeishuFormInformation(datas).then((res) => {
            ...
            .then((res) => {
              // 组件动态渲染
             myCarbondev = window.webComponent.render(
                "DocsComponent",
                {
                  //组件参数
            ...
            myCarbondev.config.setFeatureConfig({
              HEADER: {
                enable: true, //  隐藏头部
              },
              MODAL: {
            ...
            myCarbondev.event.onFullScreenModeChange(function(isFullScreen) {
            });

            myCarbondev.event.onMountSuccess(function() {
             //加载成功
            //  console.log("加载成功");
            });
            myCarbondev.event.onMountTimeout(function() {
              // 挂载文档超时
              // console.log("挂载文档超时");
            });
