...
<script>
import { editSourceData, readSourceData } from '../../../api/carbonAssetApi';
export default ({
    data() {
        return {
           id: "",
    ...
    methods: {
        handleSubmit() {
            let data = {
                ...
                sourceChannel: this.data.sourceChannel[0],
                startDate: this.data.startDate,
                status: this.data.status[0],
                submittedTime: this.data.submittedTime,
                type: this.data.type[0],
                uomType: this.data.uomType[0]
              }
            editSourceData(data).then(res => {
